angular.module('twystAppDownload', ["ngRoute"]).
    ...
    controller('MainCtrl', ['$scope', '$http', '$window', function($scope, $http, $window) {
    	...
    	function detectDevice (info) {
            var re_android = /Android/,
                re_ios = /iOS/;
            
    	    if(info.os.search(re_android) !== -1) {
                $window.location = 'https://play.google.com/store/apps/details?id=com.twyst.app.android'
    	    }
    	    else if(info.os.search(re_ios) !== -1) {
                $window.location = 'https://itunes.apple.com/us/app/twyst.in/id907877732?mt=8';
    	    }
            else {
